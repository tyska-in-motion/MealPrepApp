...
export const parseInstructionLines = (instructions?: string) =>
  (instructions || "")
    .replace(/\r\n?/g, "\n")
    // Support recipes pasted as: "1. Do this. 2. Do that." in addition to one-step-per-line input.
    .replace(/([^\n])\s+(?=\d+[.)]\s+)/g, "$1\n")
    .split(/\n+/)
    .map((line) => line.trim())
    .filter(Boolean)
...
const normalizeInstructionSteps = (instructionSteps?: InstructionStep[] | null) =>
  Array.isArray(instructionSteps)
    ? instructionSteps
        .map((step) => ({
          segments: Array.isArray(step?.segments)
            ? step.segments.filter((segment: any) => String(segment?.text || "").trim().length > 0)
            : [],
        }))
        .filter((step) => step.segments.length > 0)
    : [];

export const getDisplayInstructionSteps = (recipe?: { instructionSteps?: InstructionStep[] | null; instructions?: string | null }) => {
  const savedSteps = normalizeInstructionSteps(recipe?.instructionSteps);
  if (savedSteps.length > 0) return savedSteps;

  return parseInstructionLines(recipe?.instructions || undefined).map((step) => ({ segments: [{ type: "text" as const, text: step }] }));
};
